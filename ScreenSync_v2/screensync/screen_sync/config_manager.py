...
class ConfigManager:
    ...
    def create_default_config(self):
        ...
        # Add default KasaSettings
        self.config['KasaSettings'] = {
            'update_frequency': '50'
        }
    ...
    def get_bulbs(self):
        ...
        for section in self.config.sections():
            ...
            elif section.startswith('BulbKasa'):
                bulbs.append({
                    'type': 'Kasa',
                    'device_alias': self.config[section]['device_alias'],
                    'placement': self.config[section].get('placement', 'center'),  # Default placement is 'Center'
                    'device_id': 'Kasa',
                    'config_id' : section
                })
    ...
    def _add_magichome_bulb(self,  ip_address, placement, color_mode):
        """Adds a new MagicHome bulb configuration."""
    ...
    def _add_kasa_bulb(self, device_alias, placement):
        """Adds a new Kasa bulb configuration."""
        kasa_bulb_count = len([s for s in self.config.sections() if s.startswith('BulbKasa')])
        section_name = f'BulbKasa{kasa_bulb_count + 1}'
